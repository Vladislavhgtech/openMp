...
#pragma omp parallel default(none) shared(first_array, second_array, sum, chunk, size)
...
#pragma omp section
	...
	std::cout << "Результаты последовательного section 2 " << sum << std::endl;
...
#pragma omp section
	...
	std::cout << "Результаты section 4 " << sum << std::endl;
	std::cout << "Время выполнения  c sections 4  " << time.count() << " nanoseconds" << std::endl;
